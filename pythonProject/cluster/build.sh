...
SHOULD_BUILD_BASE="$(grep -m 1 build_base build.yml | cut -d'"' -f2)"
SHOULD_BUILD_SPARK="$(grep -m 1 build_spark build.yml | cut -d'"' -f2)"

SPARK_VERSION="$(grep -m 1 spark build.yml | cut -d'"' -f2)"
